...
This is the code for work "Toward Protocols to Improve Molecule Embedding on Odor Discrimination Task" done by Hitesh Sapkota during Internship 2022 under the supervision of Wesley Hong (Manager), saravanan sathananda manidas (Mentor), and Hongtao Yu (Onboarding Buddy). 
...
Common Arguments for Training Scripts. 
 ...
 Each .ipynb (except train_sagemaker_api.ipynb, train_sagemaker_api_multitask.ipynb),  performs following. 
  1. Calls respective .sh script. For instance, baseline.ipynb calls baseline.sh command.
  2. .sh script calls train.py (train_multitask.py)  along with the arguments required for the specific setting. Feel, free to 
     explore the used arguments. 
  3. train.py (train_multitask.py)  performs following:
  ...
  - If data is not present, it calls associated data (bushdid for single task setting and multiple for multitask setting), processes the data and stores into the data folder present in root_dir. If data folder is not already present in the root_dir, it creates it and stores the data. Specifically, concerning single task setting the data: (a) fit_samples_rep_no.pickle, (b) valid_samples_rep_no.pickle, (c) test_samples_rep_no.pickle, where rep_no is the fold (replication) number, will be stored. In case of soft label setting the data: (a) fit_samples_rep_no_softlabel.pickle, (b) valid_samples_rep_no_softlabel.pickle, (c) test_samples_rep_no_softlabel.pickle will be stored. Also, mol_atom_map.pickle (mul_atom_map_multitask.pickle) will be store to have mapping between moleucles with associated atoms. After processing all datasets will be stored under root_dir/data. In case of multitask setting, filename will be same except multitask term such as fit_samples_rep_no_softlabel_multitask.pickle. 
  - Next, the scripts creates train and validation loader and performs training by calling the SiameseNet (in Single Task) and MultitaskNet (in Multitask). Early stopping is performed during training by tracking the validation loss. The model with lowest validation loss is used as the best model and last epoch model is treated as last model. 
  ...
  - Inference output (training, validation, test) along with the training, and validation losses will be stored in the output_dir inside root directory  output_dir by default will be ./outputs/structure_encoder-odor_encoder with structure_encoder and odor_encoder as an input arguments.
...
**To generate the Table 3 (last column) of final document, run the analysis_all_models_multitask.ipynb with  input argument bushdid_only = True and des_loss_coeff = 0, od_des_coeff = 0.** 

**To generate the Table 4 of final document, run the analysis_all_models_multitask.ipynb with  input argument bushdid_only = False and des_loss_coeff = 10, od_des_coeff = 10.**

**To generate the Table 5 (last column) of final document,  run the analysis_all_models_multitask.ipynb with  input argument bushdid_only = True and des_loss_coeff = 10, od_des_coeff = 10.**
